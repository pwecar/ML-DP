...
def main():
    ...
    for _ in range(len(lr_list)):
        ...
        #plt.sca(ax1)
        ax1.plot(epsilon_list, c=color, label="lr={:.2f} σ={:.1f} c={:.1f}".format(lr_list[_], sigma_list[_], c_list[_]))
        #plt.sca(ax2)
        ax2.plot(loss_list, c=color, label="lr={:.2f} σ={:.1f} c={:.1f}".format(lr_list[_], sigma_list[_], c_list[_]))
